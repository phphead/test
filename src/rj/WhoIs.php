<?php namespace Rj; use Exception;

class WhoIs_Exception extends Exception {}
...
class WhoIs {
	...
	public static function lookupDomain($domain){
		...
		$whoisserver = empty(static::$servers[$tld]) ? '' : static::$servers[$tld];
		if( ! $whoisserver) {
	...
	public static function queryWhoisServer($whoisserver, $domain) {
		...
		if ( ! $fp = @ fsockopen($whoisserver, $port, $errno, $errstr, $timeout)) {
			throw new WhoIs_Exception("Socket Error " . $errno . " - " . $errstr);
		}
	...
	public static function LookupIP($ip) {
		...
		foreach($whoisservers as $whoisserver) {
			$result = static::queryWhoisServer($whoisserver, $ip);
